...
(function(){
  ...
  async function load(){
    if(useServer){
      try{const res=await fetch('/api/trades');trades=await res.json();trades = trades.map(normalizeScreenshots);return}catch(e){useServer=false}}
    try{trades = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]');}catch(e){trades=[]}
    trades = trades.map(normalizeScreenshots);
  }
  async function save(){
    ...
    if(panel){
      panel.innerHTML = `<h3>Trades on ${day}</h3><div class="trade-list">${list.map(t => `
        <div class="trade-card" data-id="${t.id}">
          ...
          ${t.screenshot ? `<img class="trade-screenshot" src="${t.screenshot}" alt="Trade screenshot" />` : ''}
